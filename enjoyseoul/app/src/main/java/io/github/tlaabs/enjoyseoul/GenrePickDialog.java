...
public class GenrePickDialog extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.dialog_genre_pick);
        initView();
        createListItems();
...
//        preSelect init
    ...
    private void initView(){
        ...
        submitBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                list = getActiveItemList(genreCount);
                Intent intent = new Intent();
                intent.putExtra("genre_list",list);
    ...
    private void createListItems(){
        ...
        genreCount = cursor.getCount()+1; //including allSelect
        int rowboxCount = getRowBoxCount(genreCount);

        if(cursor != null && genreCount != 0){
            cursor.moveToFirst();
            //Create Interface
            for(int i = 0; i <= rowboxCount; i++){
                ...
                for(int k = 1; k <= 3; k++){
                    ...
                    //out of range
                    if(i*ITEM_PER_ROW + k > genreCount){
                    ...
                    //allSelectBtn
                        ...
                        toggleBtn = createItem(cursor.getString(1)); //CODENAME
    ...
    private void restorePreSelect(){
        ...
        for(int i = 0 ; i < pre.size(); i++){
            for(int j = 0 ; j <= genreCount/ITEM_PER_ROW; j++){
                for(int k = 0; k < ITEM_PER_ROW; k++){
                    if(j*ITEM_PER_ROW + (k+1) > genreCount)break;
                    else
                    {
    ...
    void setAllItemOff(int genreCount){
        for(int i = 0 ; i <= genreCount/ITEM_PER_ROW; i++){
            for(int k = 0; k < ITEM_PER_ROW; k++){
                if(i*ITEM_PER_ROW + (k+1) > genreCount)break;
                else
                {
    ...
    private ArrayList<String> getActiveItemList(int genreCount)
    {
